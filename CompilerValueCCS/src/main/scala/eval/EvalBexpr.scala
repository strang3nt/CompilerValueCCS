  ...
  def eval(b: Bexpr, subst: Map[Variable, Natural]): Boolean =
    b match
      case BoolBinOp(left, rest) =>
        rest.foldLeft(eval(left, subst)){case(acc, (op, x)) => op.applyOperator(acc, eval(x, subst))}

      case BoolTerm.UnOp(op, BoolBinOp(left, rest)) =>
        rest.foldLeft(op.applyOperator(eval(left, subst))){case(acc, (op, x)) => op.applyOperator(acc, eval(x, subst))}
      ...
      case ExprBinOp(left, op, right) =>
        op.applyOperator(EvalAexpr.eval(left, subst), EvalAexpr.eval(right, subst))
