...
object EvalAexpr:
  def eval(a: Aexpr, subst: Map[Variable, Natural]): Natural =
    a match
      ...
      case Expr(t, ts) =>
        ts.foldLeft(eval(t, subst)) { case (acc, (op, x)) =>
          op.applyOperator(acc, eval(x, subst))
        }

      case Term(f, fs) =>
        fs.foldLeft(eval(f, subst)) { case (acc, (op, x)) =>
          op.applyOperator(acc, eval(x, subst))
        }
      ...
      case Factor.ID(x) =>
        throw new Exception(
          s"EvalAexpr: found variable $x: no suitable substitution found, there must not be free variables during eval."
        )
