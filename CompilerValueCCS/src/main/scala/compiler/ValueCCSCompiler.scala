        ...
        name match
          case Proc(n, Some(l)) =>
            var substCombinations: List[List[Natural]] = 
              (List.fill(l.length)(0 to maxNat))
                .flatten
                .combinations(l.length)
                .toList
                .foldLeft(List.empty)((acc, l) => acc ++ l.map(Natural(_)).permutations.toList)
            substCombinations.map(
              (nats) => PureCCSProcess(n + "_" + nats.mkString("_") , translateProcess(process, maxNat, (l zip nats).toMap)))
          case Proc(n, None) =>
            PureCCSProcess(n, translateProcess(process, maxNat, Map.empty))
      }
      .mkString("\n")

  private def translateProcess(src: V, maxNat: Int, subst: Map[Variable, Natural]): P =
    src match
      ...
      case V.InputCh (Channel(n), Some(v), p) =>
        P.Sum(
          (0 to maxNat)
            .map (i => P.InputCh(Channel(n + s"_$i"), translateProcess(p, maxNat, subst + (v -> Natural(i)))))
            .toList)
      ...
      case V.Restrict(p, l) => 
        P.Restrict(
          ...
          l.foldLeft(List.empty)
            { case(acc, Channel(n)) => acc ++ (0 to maxNat).map(i => Channel(n + s"_$i")) })
      
      case V.Redirection(p, cs) =>
        P.Redirection(
          translateProcess(p, maxNat, subst), 
          cs.foldLeft(List.empty)
            { case(acc, (Channel(n), Channel(v))) => acc ++ (0 to maxNat).map(i => (Channel(n + s"_$i"), Channel(v + s"_$i")))})
